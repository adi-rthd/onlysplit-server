    ...
    ICurrentUserService currentUser) : IGroupInvitation
{
    ...
    public async Task<List<GetGroupInvitationResponse>>
    GetGroupInvitationsAsync(
        Guid groupId,
        CancellationToken cancellationToken = default)
    {
        var invitations = await context.GroupInvitations
            ...
            .Include(x => x.InvitedByUser)
            .OrderByDescending(x => x.CreatedAt)
            .Select(x => new GetGroupInvitationResponse(
                x.Id,
                x.GroupId,
                x.Group.Name,
                x.InvitedUserId,
                $"{x.InvitedUser.FirstName} {x.InvitedUser.LastName}",
                x.InvitedUser.Email,
                x.InvitedBy,
                $"{x.InvitedByUser.FirstName} {x.InvitedByUser.LastName}",
                x.Status,
